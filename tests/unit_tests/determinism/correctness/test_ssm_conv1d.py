...
Operator-level rather than model-level, because the shape is the point. The conv's
channel-last backward reduces its weight gradient over ``micro_batch * ceil(seq_len / 128)``
blocks with ``atomicAdd``; the model cells in this directory run 32x2, which is two such
blocks -- enough to exercise the path, not enough to reliably expose a regression in it. The
shapes below give 32 and 64.

Four layers of the same contract:

* the kernel replays bit-exactly with the deterministic reduction on, and demonstrably does
  not with it off -- a control, without which a build ignoring the flag would still pass;
* the deterministic reduction sums the *same* partials as the default one, so the two agree to
  fp32 accumulation error (the one comparison here that is not bitwise, and cannot be);
* a real ``MambaMixer`` replays bit-exactly under it;
* ``deterministic_mode`` refuses to build a mixer whose conv cannot deliver that.
"""
...
# Contention is (micro_batch * seq_len) / 128, so it is the token budget that matters, not the
# split. 4 * ceil(1024 / 128) = 32 blocks for the mixer, 2 * ceil(4096 / 128) = 64 for the
...
def test_kernel_replays_bitwise(monkeypatch, deterministic):
    ...
    The ``False`` arm is a claim about the *hardware's* scheduling, so it is sized with
    margin: 64 contending blocks here, against 19 of 19 replays differing on GB300 at a shape
    with only 16. ``dx`` is written by disjoint stores and is reproducible either way.
...
def test_deterministic_reduction_agrees_with_the_default_one(monkeypatch):
    ...
    The only non-bitwise comparison in this file, and necessarily so: it compares two
    different summation orders, which fp32 cannot make equal. The bar is the gap relative to
    each tensor's own magnitude, measured at ~6e-07 (a few fp32 ulps) against the 1e-5 bound
    below. Deliberately ``rtol=0`` with the tensor magnitude in ``atol``: per element the gap
    reaches ~1e-2, because a ``dweight`` entry near zero is a near-total cancellation of much
    larger products. The cost is that this would not catch a path corrupting only entries far
    below the tensor maximum. ``dx`` skips the reduction entirely.
    """
    inputs = _channel_last_conv_inputs(**_KERNEL_SHAPE)
...
class TestMambaMixerDeterminism:
    ...
    def test_mixer_replays_bit_exactly(self, monkeypatch):
        ...
        ``MAMBA_DETERMINISTIC`` pins the SSD scan so a failure points at the convolution
        rather than the scan, whose nondeterminism would otherwise reach the conv weight
        gradient through the conv's incoming gradient.
        """
        ...
        # Drain run A's collectives and autograd post-hooks before B starts, as
        # BitExactRunner._two_runs does -- otherwise B can overlap A's tail.
    ...
    def test_deterministic_mode_requires_a_deterministic_conv(self, monkeypatch):
        """The guard is wired into mixer construction, not merely defined.

        Without this, deleting the ``assert_causal_conv1d_deterministic`` call from
        ``MambaMixer.__init__`` would leave every other test passing.
        """
        monkeypatch.setenv("CAUSAL_CONV1D_DETERMINISTIC", "0")
...
def test_guard_accepts_an_enabled_kernel(monkeypatch, env):
    ...
    ``None`` is the configuration ``--deterministic-mode`` actually produces -- env unset plus
    ``torch.use_deterministic_algorithms(True)`` -- so without it the fallback would go
    untested and a regression there would abort every deterministic run at mixer construction.
    """
...
def test_guard_rejects_what_cannot_deliver_it(monkeypatch):
    """Both failure modes raise, and neither is inherited from torch's flag alone.

    The last case matters because several unit tests set torch's flag globally and never
    restore it; treating that as a request for Megatron determinism would fail runs that never
    asked for bit-exactness.
    """
    _fake_min_version(monkeypatch, True)
    monkeypatch.setenv("CAUSAL_CONV1D_DETERMINISTIC", "0")
    with pytest.raises(AssertionError, match="deterministic causal_conv1d backward"):
