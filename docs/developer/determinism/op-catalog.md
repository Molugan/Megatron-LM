...
The two conv rows differ because they are different kernels: Mamba and GDP call
Dao-AILab's `causal_conv1d`, which has a deterministic reduction of its own,
while gated-delta-net binds FLA's, which does not and so still falls back to
`F.conv1d`.
...
`dweight` for a workspace and one `sum`, which measures between +24% and -25% of
the convolution's device time on GB300 — a fixed few microseconds at low
occupancy, and a net *saving* once enough blocks are contending.
