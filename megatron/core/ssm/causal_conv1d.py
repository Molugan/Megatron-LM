...
guard the SSM mixers apply before relying on the causal_conv1d backward."""
...
def assert_causal_conv1d_deterministic(deterministic_mode):
    ...
    ``atomicAdd``, which fixes no order; newer builds use a per-block workspace and an ordered
    reduce instead. Most visible on the channel-last layout that GDP and Mamba's fused path
    both feed the conv, where an element takes ``batch * ceil(seqlen / 128)`` contributions
    rather than the channels-first ``batch``.

    Scoped to ``deterministic_mode`` rather than torch's global flag, which gets set for
    unrelated reasons. Mirrors ``TEDotProductAttention``'s NVTE check: the env var is
    validated in ``megatron/training/determinism.py``, and the module asserts that what it is
    about to build can deliver. Call once at construction.
    """
